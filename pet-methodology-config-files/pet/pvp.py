...
class MyTaskPVP(PVP):
    ...
    def get_parts(self, example: InputExample):
        ...
        # can also be empty).
        if self.pattern_id == 0:
            return [text_a, '.', text_b, 'In summary, this is', self.mask], []
        elif self.pattern_id == 1:
            return [text_a, '.', text_b, 'You are just ', self.mask, '!'], []
        elif self.pattern_id == 2:
            return ['It was', self.mask, '.', text_a, '.', text_b], []
        elif self.pattern_id == 3:
            return ['Just', self.mask, '!', text_a, '.', text_b], []
        else:
            raise ValueError("No pattern implemented for id {}".format(self.pattern_id))
